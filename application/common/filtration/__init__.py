...
class _Filtration(object):
    ...
    def get_request_detail(self):
        ...
        return "<{f_module}.{f_class}>({method})".format(
    ...
    def parse(self):
        ...
        for item in self.items:
            ...
            if item.required and not self.is_exist_item_name(item.name):
                raise FiltrationException("{} {}: required".format(self.get_request_detail(), item.name), item.err_message)
    ...
    def convert_data_type(self, item, value):
        try:
            if item.type_ == int:
                ...
                return value
            raise FiltrationException("{} {}: type({}) not support".format(
                self.get_request_detail(), item.name, item.type_), item.err_message)
        except:
            raise FiltrationException("{} {}: type error({} required)".format(
                self.get_request_detail(), item.name, item.type_), item.err_message)
    ...
    def _parse_item(self, item, value):
        ...
        for funcs in item.validates:
            try:
                ...
                raise FiltrationException("{} {}: validate failed: {}({}) {}".format(
        ...
        for funcs in item.uses:
            try:
                ...
                raise FiltrationException("{} {}: uses failed: {}({}) {}".format(
                    self.get_request_detail(), item.name, funcs.f_, value, e), funcs.err_message)
        return value
...
class Item(object):
    ...
    def get_func_details(self, funcs):
        ...
        args = None
        err_message = None
        if len(funcs) > 1 and isinstance(funcs[1], dict):
            args = funcs[1]
        if len(funcs) == 3:
            err_message = funcs[2]
        return _ObjectDict(
            ...
            f_="<{f_module}.{f_class}.{f_name}>({args})".format(
...
class Helper(object):
    ...
    def is_valid_url(cls, val, domains=None):
        ...
        assert isinstance(domains, tuple) or isinstance(domains, list)

        pieces = urllib.parse.urlparse(val)
